<div id="__ajax_title" style="display: none">Activity Log</div>
                ...
                <div class="kt-inbox__items" data-type="inbox" id="e">
                    @foreach(json_decode(file_get_contents(storage_path().'/actions.json'), true) as $message)
                        @php
                            ...
                            if($message['type'] == 1) {
                                $type = 'Create a promo code';
                                $description = $message['data']['code']. ', '.$message['data']['usages'].' uses, '.$message['data']['sum'].' usd.';
                            }

                            if($message['type'] == 2) {
                                $type = 'Removing a promo code';
                                $description = $message['data']['code'];
                            }

                            if($message['type'] == 3) {
                                $type = 'Change promo code';
                                $description = $message['data']['code']. ', '.$message['data']['usages'].' uses, '.$message['data']['sum'].' usd.
                                    <br>Было: '.$message['data']['prev']['usages'].' uses, '.$message['data']['prev']['sum'].' usd';
                            }
                            ...
                            if($message['type'] == 5) {
                                $type = ($message['data']['type'] === 'ban' ? 'blocking' : 'Unlock').' site access';
                                $description = $data_user->username.' (id: '.$data_user->id.')';
                            }

                            if($message['type'] == 6) {
                                $type = 'Temporary blocking of chat';
                                $description = $data_user->username.' (id: '.$data_user->id.') - '.$message['data']['minutes'].' min.';
                            }

                            if($message['type'] == 7) {
                                $type = 'Creating a group of temporary promo codes';
							...
							if($message['type'] == 11) {
                                $type = 'Creation of a group of fakes';
                                $description = $message['data']['num'] . ' аккаунт.';
                            }

                            if($message['type'] == 12) {
                                $type = 'Create a fake account';
